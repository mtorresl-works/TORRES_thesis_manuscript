...
Glaser {\em et al.} \cite{glaser2015parallel} propose a scheme where many--body depletion interactions are treated in an implicit way, avoiding the consideration of an unnecessarily large number of simulated particles, while maintaining a particle--based approach instead of a more complicated effective pairwise potential. The basic idea is that, as depletants are treated as an ideal gas, i. e. assumed to be statistically independent, they can be randomly inserted on--the--go into the excluded volume surrounding a single translated or rotated colloid. The algorithm obeys detailed balance and can accept further multi--core parallel implementations.

In short words, the algorithm is implemented as follows:
