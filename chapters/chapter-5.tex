...
\fig{crystal16} shows multi--domain structures that have formed in the strong chirality regime. An observation from this figure is that the distance between rods located at the edges of these domains is considerably bigger compared to the distance between rods located inside the domains. {\em Despite the apparent large distance between adjacent membranes, the polymers remain adhered to one another.  This phenomenon can be attributed to a decrease in chiral energy when adjacent rods are not aligned in the same direction, which would ultimately lead to a reduction in surface tension between domains in contrast to the high surface tension that exists between the edges of domains and the polymer gas.}

\red{Perhaps we should clarify this statement (in italics). Are we saying that polymers accumulate in the domain walls between the subcores ? This would makes sense given that there is more free volume in the domain walls. I do not understand the connection with chiral energy and surface tension though. I guess this comment just needs to be rephrased.  }
