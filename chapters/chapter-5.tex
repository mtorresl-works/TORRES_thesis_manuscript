...
Membrane morphologies for different values of $\varepsilon_c$ are shown in \fig{multidomain} for systems with $N=500$. For comparison, we show each snapshot along with its equivalent in the case without vertical fluctuations. Chirality affects the shape of the membrane in different ways depending on whether the system is confined or not. In the confined case, where the rods are confined within a plane, the membrane shape tends to present a circular shape with slightly fluctuating boudaries. As for the unconfined case, rods are allowed to escape the horizontal plane, and thus can be found with a higher probability in the isotropic phase or adsorbed onto the membrane surface; and membranes tend to present a more elongated shape, rather than circular. These elongated structures, although they do not give way to elongated twisted ribbons, may sometimes show indications of a ribbon instability, e. g. for $\varepsilon_c=4$ in the unconfined case in \fig{multidomain}, and these instabilities are never observed in the confined case. One significant difference between the two cases is that multi-domain membranes appear at stronger chiral forces in the confined case ($\varepsilon_c=16$) than in the unconfined case ($\varepsilon_c=12$), which lead us to think that vertical ordering hinders the apparition of multi--domain structures in favor of single domain membranes with stronger twist at the edges.
...
In \fig{crystal11}, we observe stabilized twisted membranes in the intermediate chirality regime. We see that ellipses are more separated from each other in the case without vertical fluctuations, whereas in the unconfined case rods tend to occupy a smaller surface and remain closer to their neighbours. The higher local density in the unconfined case induces a higher level of crystalline order as can be observed by comparing subpanels (e) and (h); yellow regions in these panels present smaller fluctuations in the relative distance between rods. In contrast, rods in the confined case present a more liquid--like ordering. This effect may be due to the difference in the depletant concentration wich, as explained previously, is different for each case. However, for lower depletant concentrations, a smectic droplet is hard to stabilize in the unconfined case, and thus we state that a more liquid--like internal ordering is unlikely to be found in membranes of these characteristics.
...
\begin{SCfigure}
\includegraphics[width= .7\columnwidth]{figures/chapter-5/density}
...
Results are shown in \fig{density} for the confined and unconfined case, for the two values of $N$ and several values of $\varepsilon_c$ that cover the whole low to intermediate chirality regime (where multi--domain membranes are not found for any of the simulations). Results are averaged over 100 configurations at different MC cycles of the same equilibrated simulation job.


\begin{SCfigure}
\includegraphics[width= .7\columnwidth]{figures/chapter-5/twistprofile}
...
\begin{figure}
\begin{center}
\includegraphics[width= .9\columnwidth]{figures/chapter-5/zstd}
