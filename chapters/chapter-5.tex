...
\label{urod}
\eeq
The electrostatic interactions between {\em fd} rods gives rise to so-called electrostatic twist which is intimately linked to the chirality surface architecture of {\em fd} virus rods. The chiral  potential is commonly expressed in terms of a pseudoscalar form initially put forward by Goossens \cite{goossens}: % TODO: reference Goossens
\beq
...
\beq
 ...
 \eeq
 While for the single-twisted cholesteric phase the twist angle increases linearly  $\varphi(z) = q_{0} z$ at each position $z$ along the helix axis, the angle will be strongly non-linear with radius $R$ in case of the double-twisted membrane, in particular if the twist penetration length $\lambda_{t}$ is small \cite{wensink2018elastic}.   
...
\caption{ \label{ribsnap} Schematic structure of of twisted ribbon of width $s_y=4\ell$ composed of strongly chiral spherocylinders with aspect ratio $\ell = 10$ mixed with non-adsorbing polymers (not shown) providing strong depletion attraction between the rods. Color gradient indicates the angular deviation with respect to the untwisted director $\bn_{0}$. Twist is expressed  in both directions by the angles $\Phi = qy$ and $\chi = q \gamma z$.}
\end{center}
\end{figure}
