...
\chaptermark{Hybrid chiral nematics I: Collective effects}
...
Combining all free energy contributions and formally minimizing the Helmholtz free energy with respect to $f_{q}$ yields a Boltzmann distribution similar to \eq{fsingle}:
  ...
  The  brackets are shorthand for an orientational average $ \langle  \cdot  \rangle_{f_{q}} = \int d \bhu   f_{q}(\bhu )(\cdot)$ measured with respect to the local director.
   ...
   \subsection{Weak coupling regime: orthorhombic-orthorhombic phase separation}
...
% TODO: rearrange figure to fit 1-column
   \begin{figure}
...
\section{Effect of twist elastic resistance}
...
\eeq
% TODO: properly reference the desired figure
In the absence of twist elastic effects the surface anchoring energy is indeed minimized along a uniform twist profile $\phi(z) = qz + \phi_{0} $ with phase shifts $\phi_{0} = \pi/2$ (H/T) and $\phi_{0} = 0$ (SP) as evident from the result in Fig. 1 and 2.
...
in terms of an amplitude $\delta \phi$ and periodicity $q_{b}$. The amplitude slowly decays as $q \xi \rightarrow \infty $ while the breathing periodicity attains a constant value $q_{b}/q \rightarrow 2$.
   ...
   \begin{figure*}
	\includegraphics[width = \columnwidth]{figures/chapter-5/ripplediagram}
...
\end{figure*}
 ...
 \begin{figure}
	\includegraphics[width = .8\columnwidth]{figures/chapter-5/fluctuations}
...
Another common solution that is associated with the sine-Gordon equation is the soliton. Taking the boundary conditions $\varepsilon^{\prime}(\pm \infty) = 0$ and $\varepsilon(\infty) - \varepsilon(-\infty)  = \pi$. The solution for the single soliton can be obtained in analytical form  [Kamien \& Selinger2001]:
\beq
...
\section*{Appendix: twist elastic resistance of thin hard needles}

For   hard rods  a tractable analytical expression for the $z-$resolved excluded area is available in the needle limit $L/D \rightarrow \infty$ [Poniewierski, Shundyak]:
\begin{align}
