...
\red{\fig{crystal16} shows multi--domain structures that have formed in the strong chirality regime. It is observed from the horizontal section depictions that the distance between rods located at the edges of these domains is considerably bigger compared to the distance between rods located inside the domains. This is due to the highly non optimal collective arangement (from a free volume perspective) rods adopt at the edges. Despite the apparent large distance between adjacent domains --that may allow polymers to accumulate-- they remain adhered to one another presumably due to a significant decrease in chiral energy when adjacent rods are close together and forming an optimal angle. }
