...
\end{align}
We now simplify matters by assuming linear twist in both directions so that $\chi = q_{1}z$ and $\chi = q_{2}y$ in terms of a longitudinal pitch $q_{1}$ and transverse pitch $q_{2}$ denoting the degree of twist along the long and short ribbon axes, respectively. We remark that these pitches are not necessarily the same. They are also likely to be different from the membrane pitch $q_{0}$ that we defined in \eq{qzero} which quantifies the chiral strength between the rods.

Since the ribbon consists of repetitive units of length $s_{z} = 2\pi/q$ and width $s_{y}$ we focus our attention on the free energy per unit cell. First, the  elastic contributions corresponding to bens, splay and twist read:
\beq
F_{bend} =K_{3}
\frac{\pi \left[ 2 q_{1}^3+2 q_{1}^2 \sin q_{1}+q_{1}
   q_{2}^2-q_{2}^2 \sin q_{1} \cos q_{1} \right ]}{8 q_{1}
   q_{2}} 
\eeq
\beq
F_{splay} =  K_{1}
\frac{\pi q_{1}  \left[ q_{1} - \sin q_{1} \right ]  }{4 q_{2}}  
\eeq
\beq
F_{twist} = K_{2} \frac{\pi  \left [ q_{1} \left(8 q_{0}^2-8 q_{0} q_{2}+4 q_{1}^2+3
   q_{2}^2\right)+q_{2} \sin q_{1} (q_{2} \cos
   q_{1}+4 q_{2}-8 q_{0})\right ] }{8 q_{1} q_{2}}
\eeq
